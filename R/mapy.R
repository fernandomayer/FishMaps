mapy <- function(year, lat, lon, cfu, breaks, type = c("LL","BB"), ident = TRUE,
    ...
    fig.par = NULL, ...)
{
    ...
    # extract the levels (uniques) of the year
    if(is.factor(year))
    {
            ...
            LL = {    
               	...
               	for(i in lev.year)
            	{
        		    ...
        		    if(ident == TRUE)
        		    {
        		        ...
        		        switch(ident.type,
        		            num = {text(par("usr")[1],par("usr")[4], i, font=2, adj=c(-0.4,1.5),
        		                cex=ident.cex)},
        		    ...
        		    if(any(cfu == 0))
        		    {
        		        points(lon[year==i & cfu==0], lat[year==i & cfu==0], pch=3, cex=1)
        		        points(lon[year==i & cfu!=0], lat[year==i & cfu!=0], pch=19,
        		            cex=points.idx[year==i & cfu!=0])
        		        # legend
        		        if(legend == TRUE)
        		        {
        		    ...
        		    {
        		        points(lon[year==i], lat[year==i], pch=19, cex=points.idx[year==i])
        		        # legend
        		        if(legend == TRUE)
        		        {
            		        ifelse(length(breaks) > 1,
